...
test("rejects an invalid install without changing installed configuration", async ({ page, base }) => {
  ...
  await expect(page.getByRole("textbox", { name: "GitHub repository URL" })).toBeHidden()
...
for (const viewport of [{ name: "desktop", width: 1280, height: 800 }, { name: "mobile", width: 390, height: 844 }]) {
  test(`${viewport.name} settings shows configured skills with usable navigation`, async ({ page, base }) => {
    ...
    await page.getByRole("button", { name: "Install Agentic PM pack" }).scrollIntoViewIfNeeded()
    await expect(page.getByRole("textbox", { name: "GitHub repository URL" })).toBeHidden()
    if (process.env.UPDATE_CAPABILITY_PACK_SCREENSHOTS === "1") {
      await page.screenshot({ path: path.join("public/screenshots/docs", `capability-packs-install-${viewport.name}.png`), animations: "disabled" })
      await page.getByRole("button", { name: "Advanced", exact: true }).click()
      await page.getByRole("textbox", { name: "Pack path" }).scrollIntoViewIfNeeded()
      await page.screenshot({ path: path.join("public/screenshots/docs", `capability-packs-advanced-${viewport.name}.png`), animations: "disabled" })
      await page.getByRole("button", { name: "Advanced", exact: true }).click()
    }
    await expect(page.getByRole("checkbox", { name: "discovery", exact: true })).toBeChecked()
...
test("one click pins the curated pack and preserves disabled installation on reload", async ({ page, base }) => {
  test.skip(process.env.E2E_CURATED_CAPABILITY_PACK !== "1", "Opt in to the live curated GitHub source")
  await clearPacks()
  await page.goto(`${base}/settings`)
  await expect(page.getByRole("textbox", { name: "GitHub repository URL" })).toBeHidden()
  await page.getByRole("button", { name: "Install Agentic PM pack" }).click()
  await expect(page.getByRole("button", { name: "Agentic PM pack installed" })).toBeDisabled({ timeout: 60_000 })
  const installed = await pool.query(`SELECT v.source_commit, v.source_repository, v.source_path, a.capability_pack_version_id FROM compass_dev.workspace_capability_packs a JOIN compass_dev.capability_pack_versions v ON v.id=a.capability_pack_version_id WHERE a.workspace_id=$1`, [workspaceId])
  expect(installed.rows).toHaveLength(1)
  expect(installed.rows[0].source_commit).toMatch(/^[a-f0-9]{40}$/)
  expect(installed.rows[0].source_repository).toBe("https://github.com/rbcodelabs/agent-pm-playbook")
  expect(installed.rows[0].source_path).toBe("packs/compass")
  await page.getByRole("button", { name: "Disable", exact: true }).click()
  await expect(page.getByRole("button", { name: "Enable", exact: true })).toBeVisible()
  await page.reload()
  await expect(page.getByRole("button", { name: "Agentic PM pack installed" })).toBeDisabled()
  await expect(page.getByRole("button", { name: "Enable", exact: true })).toBeVisible()
  const retained = await pool.query(`SELECT enabled, capability_pack_version_id FROM compass_dev.workspace_capability_packs WHERE workspace_id=$1`, [workspaceId])
  expect(retained.rows[0]).toEqual({ enabled: false, capability_pack_version_id: installed.rows[0].capability_pack_version_id })
})
