 ...
 *          confirm the link persisted → confirm the Overview/Subtasks/Links
 *          tabs all render.
 *
...
test.describe("Tasks", () => {
  test(
    "create task → drag to a new status column → link to an opportunity → detail page tabs",
    async ({ page, base }) => {
      const ts = Date.now();
