...
const mockPrisma = { roadmapItem: mockRoadmapItem, launchChecklistItem: mockLaunchChecklistItem };
...
describe("setLaunchTier", () => {
  it("throws Unauthorized when there is no session", async () => {
    ...
    await expect(setLaunchTier(ITEM_ID, "TIER_1", WS, PATH)).rejects.toThrow("Unauthorized");
  ...
  it("throws when the roadmap item isn't in the workspace", async () => {
    ...
    await expect(setLaunchTier(ITEM_ID, "TIER_1", WS, PATH)).rejects.toThrow("Roadmap item not found");
    expect(setLaunchTierCore).not.toHaveBeenCalled();
  ...
  it("rejects an item that is already LAUNCHING without writing", async () => {
    ...
    await expect(setLaunchTier(ITEM_ID, "TIER_1", WS, PATH)).rejects.toThrow(/already LAUNCHING/);
    expect(setLaunchTierCore).not.toHaveBeenCalled();
  });
  ...
  it("rejects an item that is already LAUNCHED without writing", async () => {
    ...
    await expect(setLaunchTier(ITEM_ID, "TIER_1", WS, PATH)).rejects.toThrow(/already LAUNCHED/);
    expect(setLaunchTierCore).not.toHaveBeenCalled();
  });

  it("resolves the template, runs the core, and revalidates on success", async () => {
    await setLaunchTier(ITEM_ID, "TIER_1", WS, PATH);
...
describe("updateLaunchChecklistItem", () => {
  it("throws Unauthorized when there is no session", async () => {
    mockAuth.mockResolvedValueOnce(null as unknown as Session);
    await expect(updateLaunchChecklistItem("lci-1", "DONE", WS, PATH)).rejects.toThrow("Unauthorized");
  ...
  it("throws when the checklist item isn't owned by the workspace", async () => {
    ...
    await expect(updateLaunchChecklistItem("lci-1", "DONE", WS, PATH)).rejects.toThrow("Checklist item not found");
    expect(updateChecklistItemCore).not.toHaveBeenCalled();
    expect(revalidatePath).not.toHaveBeenCalled();
  });

  it("scopes ownership through the checklist → roadmapItem → workspace chain", async () => {
    await updateLaunchChecklistItem("lci-1", "DONE", WS, PATH);
