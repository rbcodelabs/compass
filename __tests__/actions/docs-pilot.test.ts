...
const nav = vi.hoisted(() => ({ redirect: vi.fn((url: string) => { throw Object.assign(new Error("NEXT_REDIRECT"), { url }) }) }))
vi.mock("next/navigation", () => ({ redirect: nav.redirect, notFound: vi.fn() }))
...
import { updateDoc, createDoc, createFirstDoc, getDocVersionContent, listDocComments } from "@/app/[orgSlug]/[workspaceSlug]/docs/actions"
...
it("empty-state creation sends a fresh operation ID and opens the new page", async () => {
  // Pilot workspaces reject creates without an operation ID (operation-id-required → HTTP 500).
  await expect(createFirstDoc("org", "space")).rejects.toThrow("NEXT_REDIRECT")
  expect(m.create).toHaveBeenCalledWith({ workspaceId: "workspace", parentId: null, title: "Untitled" }, expect.objectContaining({ operationId: expect.stringMatching(/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/), authorId: "user" }))
  expect(nav.redirect).toHaveBeenCalledWith("/org/space/docs/doc")
})
it("empty-state creation refuses a non-member before touching storage", async () => {
  m.workspace.mockResolvedValue(null)
  await expect(createFirstDoc("org", "space")).rejects.toThrow()
  expect(m.create).not.toHaveBeenCalled()
})
