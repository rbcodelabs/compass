...
vi.mock("@/components/roadmap/roadmap-gantt", () => ({ RoadmapGantt: () => null }));
...
describe("server-owned native timeline rollout", () => {
  it("routes the allowed workspace timeline to native and remounts only when squad changes", async () => {
  ...
  it("keeps an explicit classic fallback", async () => { expect(find(await render({ view: "timeline", timelineEngine: "classic" }), RoadmapGantt)).toBeDefined(); });
  it("cannot enable native in another workspace via query or copied slugs", async () => {
    workspace.id = "other-workspace";
    try { expect(find(await render({ view: "timeline", timelineEngine: "native" }), RoadmapGantt)).toBeDefined(); }
