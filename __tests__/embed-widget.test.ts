...
describe("rendering a thread", () => {
  ...
  it("tells an anonymous reader that signing in may help, without asserting it will", async () => {
    // The same 403 reaches two different callers. To someone with no identity, the
    // only useful next step is to try theirs; to someone already signed in, the
    // workspace setting is the whole explanation. Both statements have to be true
    // without the widget knowing which side of the gate the reader is on.
    on("GET /api/embed/comments", reply(403, { error: "Feedback on this workspace's artifacts is not publicly readable." }))
    await mount()

    const text = part("read-blocked").textContent as string
    expect(text).toContain("sign in")
    expect(text).toContain("leave new feedback")
    // Not the signed-in wording: nothing here claims publishing is the reason.
    expect(text).not.toContain("not published")
  })

  it("re-reads the thread once sign-in completes, since membership may now allow it", async () => {
    vi.useFakeTimers()
    // Refused while anonymous, allowed once the caller presents an internal
    // identity that is a member of the workspace. Without dropping the latch the
    // member would keep the banner until they reloaded the host page.
    let signedIn = false
    on("GET /api/embed/comments", () =>
      signedIn
    ...
    expect(lastCall("GET /api/embed/comments").headers["X-Compass-Visitor"]).toBe(VISITOR)
    expect(maybePart("read-blocked")).toBeNull()
    // An empty thread rather than a refusal: the read succeeded this time.
    expect(part("empty").textContent).toContain("No feedback")
  })

  it("drops a thread it can no longer fetch when the visitor signs out", async () => {
    storeVisitor()
    on("GET /api/embed/session", reply(200, { signedIn: true, email: "ada@example.com", name: "Ada" }))
    ...
    on("GET /api/embed/comments", () =>
      signedIn
        ? { status: 200, data: { artifactId: "art_1", comments: [comment({ body: "Members only." })] } }
        : { status: 403, data: { error: "Feedback on this workspace's artifacts is not publicly readable." } }
    )
    await mount()
    expect(part("thread").textContent).toContain("Members only.")

    signedIn = false
    click(part("signout"))
    await flush()

    // Left on screen it would be a thread the widget cannot refresh and the reader
    // is no longer entitled to — stale in the one direction that matters.
    expect(maybePart("thread")).toBeNull()
    expect(shadow().textContent).not.toContain("Members only.")
    expect(maybePart("read-blocked")).not.toBeNull()
  })
