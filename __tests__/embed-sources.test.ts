...
describe("resolveEmbedToken", () => {
  ...
  it("reads a NULL auth mode as INTERNAL_SSO rather than leaving it unset", async () => {
    // The column is nullable with no database default — Aurora DSQL cannot attach
    // one to an ADD COLUMN — so every source that predates it reads as NULL here.
    // Resolving that to INTERNAL_SSO is what makes those rows usable at all, since
    // PORTAL needs email delivery this deployment does not have.
    mockToken.findUnique.mockResolvedValue(
      storedToken({ feedbackSource: { ...storedToken().feedbackSource, authMode: null } })
    );
    await expect(resolveEmbedToken(RAW_TOKEN)).resolves.toMatchObject({ authMode: "INTERNAL_SSO" });
  });

  it("reads an unrecognized auth mode as INTERNAL_SSO instead of failing open", async () => {
    // A garbled column must not widen who may write. INTERNAL_SSO demands
    // workspace membership, which is strictly stronger than PORTAL's bar, so the
    // stricter mode is the safe landing place for a value nobody can interpret.
    for (const stored of ["portal", "ANYONE", ""]) {
      mockToken.findUnique.mockResolvedValue(
        storedToken({ feedbackSource: { ...storedToken().feedbackSource, authMode: stored } })
      );
      await expect(resolveEmbedToken(RAW_TOKEN)).resolves.toMatchObject({ authMode: "INTERNAL_SSO" });
    }
  });
