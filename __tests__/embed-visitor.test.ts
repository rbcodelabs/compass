 ...
 * claimed exactly once, that the well-formedness gate runs before any query, and
 * that a credential names exactly one identity — never both, never neither.
 */
...
/** The same row, as an internal member's session rather than a portal reviewer's. */
function storedInternalSession(overrides: Record<string, unknown> = {}) {
  return storedSession({
    portalAccountId: null,
    portalAccount: null,
    userId: "user-1",
    user: { id: "user-1", email: "pm@bankrate.com", name: "Pat" },
    ...overrides,
  });
}
...
describe("mintEmbedVisitorSession", () => {
  it("returns the raw token once and stores only its digest", async () => {
    const { token } = await mintEmbedVisitorSession({
      feedbackSourceId: "source-1",
      identity: { portalAccountId: "account-1" },
    });
  ...
  it("mints a distinct token each time", async () => {
    const first = await mintEmbedVisitorSession({ feedbackSourceId: "s", identity: { portalAccountId: "a" } });
    const second = await mintEmbedVisitorSession({ feedbackSourceId: "s", identity: { portalAccountId: "a" } });
    expect(first.token).not.toBe(second.token);
  });

  it("writes exactly one identity column, whichever kind it was given", async () => {
    // The column the other kind would use is written as an explicit null rather
    // than omitted. Both are nullable — there is no database constraint holding
    // "exactly one of these", so the write is where the invariant is established
    // and the read path (below) is where it is re-checked.
    await mintEmbedVisitorSession({ feedbackSourceId: "s", identity: { portalAccountId: "account-1" } });
    expect(mockSession.create.mock.calls[0][0].data).toMatchObject({
      portalAccountId: "account-1",
      userId: null,
    });

    await mintEmbedVisitorSession({ feedbackSourceId: "s", identity: { userId: "user-1" } });
    expect(mockSession.create.mock.calls[1][0].data).toMatchObject({
      portalAccountId: null,
      userId: "user-1",
    });
  });
...
describe("resolveEmbedVisitorToken", () => {
  ...
  it("resolves an internal session to the Compass user behind it", async () => {
    // `kind` is what the write path branches on to decide between a real
    // Comment.authorId and a CommentExternalAuthor shim, so it is part of the
    // contract rather than a convenience.
    mockSession.findUnique.mockResolvedValue(storedInternalSession());

    await expect(resolveEmbedVisitorToken(RAW_TOKEN, "source-1")).resolves.toEqual({
      kind: "INTERNAL",
      userId: "user-1",
      email: "pm@bankrate.com",
      name: "Pat",
    });
  });

  it("refuses a row naming both identities, rather than picking one", async () => {
    // DSQL has no CHECK constraint to hold exactly-one-of (relationMode="prisma"),
    // so a malformed row is reachable in principle. Guessing which of two people
    // wrote a comment is worse than refusing the credential.
  ...
  it("refuses a row naming neither identity", async () => {
    mockSession.findUnique.mockResolvedValue(
      storedSession({ portalAccountId: null, portalAccount: null, userId: null, user: null })
    );
    await expect(resolveEmbedVisitorToken(RAW_TOKEN, "source-1")).resolves.toBeNull();
  });

  it("refuses a session whose account or user has since been deleted", async () => {
    // Under relationMode="prisma" the id can outlive the row it points at, and an
    // identity that cannot be named cannot author a comment.
    mockSession.findUnique.mockResolvedValue(storedSession({ portalAccount: null }));
    await expect(resolveEmbedVisitorToken(RAW_TOKEN, "source-1")).resolves.toBeNull();

    mockSession.findUnique.mockResolvedValue(storedInternalSession({ user: null }));
    await expect(resolveEmbedVisitorToken(RAW_TOKEN, "source-1")).resolves.toBeNull();
  });
...
describe("depositEmbedAuthHandoff", () => {
  ...
  it("deposits an internal identity the same way, and no portal account", async () => {
    await depositEmbedAuthHandoff({
      nonce: NONCE,
      feedbackSourceId: "source-1",
      identity: { userId: "user-1" },
    });
    expect(mockHandoff.create.mock.calls[0][0].data).toMatchObject({
      userId: "user-1",
      portalAccountId: null,
    });
  });
...
describe("claimEmbedAuthHandoff", () => {
  ...
  it("carries an internal identity from deposit through to the minted session", async () => {
    mockHandoff.findUnique.mockResolvedValue(
      storedHandoff({ portalAccountId: null, userId: "user-1" })
    );
  ...
  it("refuses a handoff naming both identities, having burned it anyway", async () => {
    mockHandoff.findUnique.mockResolvedValue(storedHandoff({ userId: "user-1" }));

    await expect(claimEmbedAuthHandoff({ nonce: NONCE, feedbackSourceId: "source-1" })).resolves.toBeNull();
    // Burned regardless: it has been on the wire, and the visitor's recovery is to
    // sign in again rather than to re-present it.
    expect(mockHandoff.deleteMany).toHaveBeenCalled();
    expect(mockSession.create).not.toHaveBeenCalled();
  });

  it("refuses a handoff naming neither identity", async () => {
    mockHandoff.findUnique.mockResolvedValue(storedHandoff({ portalAccountId: null, userId: null }));
