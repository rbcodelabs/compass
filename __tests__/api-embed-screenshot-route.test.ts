...
describe("POST /api/embed/screenshot — an internal visitor", () => {
  beforeEach(() => {
    mockResolve.mockResolvedValue({ ...RESOLVED, authMode: "INTERNAL_SSO" });
    mockVisitor.mockResolvedValue({ ...INTERNAL_VISITOR });
  });

  it("accepts a capture from a member of the source's workspace", async () => {
    const response = await POST(post({ dataUrl: PNG_DATA_URL }));
    expect(response.status).toBe(201);
    expect(mockIsMember).toHaveBeenCalledWith("ws-1", "user-1");
  });

  it("401s a visitor whose membership was revoked, without buffering the body", async () => {
    // This endpoint spends Compass's blob storage, so the re-check is not skipped
    // as "only an image" — and it runs before the body is read, same as the
    // missing-token case.
    mockIsMember.mockResolvedValue(false);

    const request = post({ dataUrl: PNG_DATA_URL });
    const response = await POST(request);
    expect(response.status).toBe(401);
    await expect(response.json()).resolves.toMatchObject({ code: "PORTAL_AUTH_REQUIRED" });
