...
describe("preview cleanup", () => {
  it("rejects a deployment mismatch without deleting anything", async () => {
    ...
    await expect(cleanupPreviewRun(prisma as unknown as PrismaClient, "run", "deployment")).rejects.toThrow();
  ...
  it("treats a missing run as already cleaned", async () => {
    ...
    expect(await cleanupPreviewRun(prisma as unknown as PrismaClient, "run", "deployment")).toEqual({ runId: "run", cleaned: true });
  });
});
