...
describe("participant evidence owned cleanup", () => {
  it("scopes child deletion to the exact owned workspace", async () => {
    ...
    await deleteParticipantVoiceEvidenceIfPresent({ researchParticipantVoiceEvent: { deleteMany } } as unknown as AppPrismaClient, "workspace")
  ...
  it("allows pre-049 schema but propagates all other cleanup failures", async () => {
    ...
    const prisma = { researchParticipantVoiceEvent: { deleteMany } } as unknown as AppPrismaClient
