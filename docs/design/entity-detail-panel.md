...
- **PR 4 — Inline editing** ✅: status/horizon dropdown + inline-editable title and description in all 8 panels. A scoped `updateEntityField` + PATCH on the same route (same workspace-membership boundary as reads — a write can no more cross workspaces than a read can). Members can edit. Also fixed the opportunity panel's stale status labels (VALIDATED/DEPRIORITIZED → the real EXPLORING/VALIDATING/PRIORITIZED/ACTIVE/ARCHIVED).
- **PR 5 — Polish** ✅: functional E2E spec (open → relation hop → back-close → inline title edit persists → deep-link restore), a user-facing help page (`docs/content/14-detail-panel.md`), and an a11y/mobile pass (Escape/focus via the Sheet; full-height mobile sheet). The E2E surfaced and fixed a real bug: `EditableText` read its new value from React state, so a paste-then-Enter (or programmatic fill) could commit the stale value — now it reads the input via a ref at commit time.
