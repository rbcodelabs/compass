...
| `update_roadmap_item` | Update a roadmap item's ordinary horizon, status, title, description, dates, `isPrivate` flag, or linked Solution. Pass `solutionId` to attach or replace the link; omit it to preserve the current link. NOW behaves like other ordinary horizons; LAUNCHING/LAUNCHED use the launch workflow (rejected here — see below — and gated by the workspace's Marketing launch setting) |
