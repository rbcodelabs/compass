...
import { assertAgentScopedOAuthBindingMigration } from "@/lib/migrations/agent-scoped-oauth-binding";
import { assertOAuthForcedReconsentMigration } from "@/lib/migrations/oauth-forced-reconsent";
import { assertOAuthAuthorizationEventsMigration } from "@/lib/migrations/oauth-authorization-events";
...
const MIGRATIONS: readonly MigrationEntry[] = [
  ...
  {
    name: "056_agent_scoped_oauth_binding",
    filePath: path.join(process.cwd(), "prisma/migrations/056_agent_scoped_oauth_binding/migration.sql"),
  },
  {
    // Must stay ordered after 056 and must not be applied without the stage-2
    // consent screen deployed: it revokes every live OAuth token, and the
    // screen is the only thing that can mint a replacement.
    name: "057_oauth_forced_reconsent",
    filePath: path.join(process.cwd(), "prisma/migrations/057_oauth_forced_reconsent/migration.sql"),
  },
  {
    name: "058_oauth_authorization_events",
    filePath: path.join(process.cwd(), "prisma/migrations/058_oauth_authorization_events/migration.sql"),
  },
  {
    name: "059_geode_document_storage",
    filePath: path.join(process.cwd(), "prisma/migrations/059_geode_document_storage/migration.sql"),
  },
  {
    name: "060_experiment_research_study_links",
    filePath: path.join(process.cwd(), "prisma/migrations/060_experiment_research_study_links/migration.sql"),
  },
  {
    name: "060_workspace_updates",
    filePath: path.join(process.cwd(), "prisma/migrations/060_workspace_updates/migration.sql"),
  },
  {
    name: "061_product_analytics",
    filePath: path.join(process.cwd(), "prisma/migrations/061_product_analytics/migration.sql"),
  },
];
...
const ASYNC_WAIT_MIGRATIONS = [...DECISION_GATE_MIGRATIONS, "047_research_voice_control_plane", "049_agent_identity", "049_research_participant_voice", "050_pm_interviews", "051_pm_agent_handoff", "052_research_evidence_promotion", "053_shared_field_option_sets", "054_webauthn_authenticators", "055_oauth_authorization_server", "056_agent_scoped_oauth_binding", "058_oauth_authorization_events", "059_geode_document_storage", "060_experiment_research_study_links", "060_workspace_updates", "061_product_analytics"] as const;
...
export async function applyMigrations(pool: Pool, schema: string, targetScript?: string, options: { preProvisionedSchema?: boolean; legacyDecisionRepairManifest?: LegacyDecisionRepairManifest } = {}) {
  ...
  try {
    ...
    for (const migration of toRun) {
      ...
      for (const stmt of statements) {
        try {
          ...
          if (!process.env.DATABASE_URL && ASYNC_WAIT_MIGRATIONS.includes(migration.name as typeof ASYNC_WAIT_MIGRATIONS[number]) && /(?:CREATE\s+(?:UNIQUE\s+)?INDEX\s+ASYNC|ALTER\s+TABLE\s+ASYNC)/i.test(stmt)) {
            ...
            if (!jobId && ["049_agent_identity", "050_pm_interviews", "051_pm_agent_handoff", "052_research_evidence_promotion", "053_shared_field_option_sets", "054_webauthn_authenticators", "055_oauth_authorization_server", "056_agent_scoped_oauth_binding", "058_oauth_authorization_events", "059_geode_document_storage", "060_experiment_research_study_links", "061_product_analytics"].includes(migration.name)) continue
      ...
      if (migration.name === "056_agent_scoped_oauth_binding") await assertAgentScopedOAuthBindingMigration(client, schema)
      ...
      if (migration.name === "060_experiment_research_study_links") await assertExperimentResearchStudyLinksMigration(client, schema)
