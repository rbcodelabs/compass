...
const MIGRATIONS = [
  ...
  {
    // Second independently-chosen "052", same as the 024/034/047/049/050/051
    // pairs above: receipts key on the full migration name, never the numeric
    // prefix, so both coexist. Not renumbered.
    name: "052_shared_field_option_sets",
    filePath: path.join(process.cwd(), "prisma/migrations/052_shared_field_option_sets/migration.sql"),
  },
];
...
const ASYNC_WAIT_MIGRATIONS = [...DECISION_GATE_MIGRATIONS, "047_research_voice_control_plane", "049_agent_identity", "049_research_participant_voice", "050_pm_interviews", "051_pm_agent_handoff", "052_research_evidence_promotion", "052_shared_field_option_sets"] as const;
...
export async function applyMigrations(pool: Pool, schema: string, targetScript?: string, options: { preProvisionedSchema?: boolean; legacyDecisionRepairManifest?: LegacyDecisionRepairManifest } = {}) {
  ...
  try {
    ...
    for (const migration of toRun) {
      ...
      for (const stmt of statements) {
        try {
          ...
          if (!process.env.DATABASE_URL && ASYNC_WAIT_MIGRATIONS.includes(migration.name as typeof ASYNC_WAIT_MIGRATIONS[number]) && /(?:CREATE\s+(?:UNIQUE\s+)?INDEX\s+ASYNC|ALTER\s+TABLE\s+ASYNC)/i.test(stmt)) {
            ...
            if (!jobId && ["049_agent_identity", "050_pm_interviews", "051_pm_agent_handoff", "052_research_evidence_promotion", "052_shared_field_option_sets"].includes(migration.name)) continue
      ...
      if (migration.name === "052_shared_field_option_sets") await assertSharedFieldOptionSetsMigration(client, schema)
