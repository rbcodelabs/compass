...
/**
 * The column shape 064_embed_feedback_sources produces, and which
 * 064_embed_feedback_sources must reproduce exactly.
 *
 * Shared between the two postconditions on purpose: 064's only job is to bring a
 * schema that ran an earlier revision of 062 onto the same shape, so "the same
 * shape" has to be one list rather than two that can drift apart.
 *
 * Nullability is asserted column by column rather than just presence, because
 * several of the choices here are load-bearing and a silently-wrong one would only
 * surface much later:
 *
 ...
 *   unbound path.
 * - `feedback_sources.auth_mode` MUST be nullable. Null reads as INTERNAL_SSO in
 *   the application layer (lib/embed-auth-mode.ts), and it has to be nullable
 *   because DSQL forbids `ADD COLUMN` from carrying a DEFAULT — which is the only
 *   way 064 can add it to an already-migrated schema. NOT NULL here would mean the
 *   two schemas disagreed about the column.
 ...
 * - `portal_account_id` and `user_id` on both credential tables MUST be nullable,
 *   and this is the pair most likely to be "corrected" by someone who has not read
 *   why. Exactly one of the two is set per row — an external reviewer or an
 *   internal SSO member — so a NOT NULL on either would forbid the other kind of
 *   identity outright. The invariant is enforced in lib/embed-visitor.ts because
 *   DSQL under relationMode="prisma" offers no FK or CHECK to carry it.
 */
const EMBED_FEEDBACK_EXPECTED_COLUMNS = [
  ...
  { name: "feedback_sources.auth_mode", nullable: true },
  ...
  { name: "embed_visitor_sessions.portal_account_id", nullable: true },
  { name: "embed_visitor_sessions.user_id", nullable: true },
  ...
  { name: "embed_auth_handoffs.portal_account_id", nullable: true },
  { name: "embed_auth_handoffs.user_id", nullable: true },
  { name: "embed_auth_handoffs.expires_at", nullable: false },
  { name: "embed_auth_handoffs.created_at", nullable: false },
]

/**
 * Asserts the shared tables/columns/indexes shape, attributing any failure to
 * whichever migration invoked it.
 *
 * `migrationName` is threaded through the messages rather than hardcoded so that a
 * failure names the statement that was actually running — a 064 convergence that
 * left a column NOT NULL is a different bug from a 062 that never created it, and
 * a message reading "062" while 064 was executing would send the next reader to
 * the wrong file.
 */
async function assertEmbedFeedbackShape(client: PoolClient, schema: string, migrationName: string) {
  ...
  for (const expected of EMBED_FEEDBACK_EXPECTED_COLUMNS) {
    ...
    if (!actual || (actual.is_nullable === "YES") !== expected.nullable) {
      throw new Error(`${migrationName}: column postcondition failed: ${expected.name}`)
    }
  }
  ...
  if (missing.length > 0) throw new Error(`${migrationName}: missing tables: ${missing.join(", ")}`)
  ...
  if (unfinished.length > 0) {
    throw new Error(`${migrationName}: missing or unfinished indexes: ${unfinished.join(", ")}`)
  }
  ...
  if (!indexes.rows.find(row => row.name === "idx_feedback_source_tokens_hash")?.unique) {
    throw new Error(`${migrationName}: idx_feedback_source_tokens_hash is not unique`)
  }
  ...
  for (const name of ["idx_embed_visitor_sessions_hash", "idx_embed_auth_handoffs_nonce"]) {
    if (!indexes.rows.find(row => row.name === name)?.unique) {
      throw new Error(`${migrationName}: ${name} is not unique`)
    }
  }
}

/** Postcondition for 064_embed_feedback_sources. */
export async function assertEmbedFeedbackSourcesMigration(client: PoolClient, schema: string) {
  await assertEmbedFeedbackShape(client, schema, "064_embed_feedback_sources")
  ...
  if (flag.rows[0]?.is_nullable !== "YES") {
    throw new Error("064_embed_feedback_sources: workspaces.artifact_feedback_public missing or not nullable")
  }
}
