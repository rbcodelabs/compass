...
export function researchFailureDiagnostic(error: unknown): { category: string; status?: number } {
  const value = error && typeof error === "object" ? error as { status?: unknown; statusCode?: unknown; code?: unknown; name?: unknown; message?: unknown } : {}
  ...
  const category = status === 401 || /authentication_error|invalid.api.key/i.test(message) ? "authentication_failed"
    ...
    : ["ECONNRESET", "ECONNREFUSED", "ENOTFOUND"].includes(String(value.code)) ? "network_failure"
    ...
    : /not initialized|not configured/i.test(message) ? "configuration_missing"
