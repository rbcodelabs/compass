...
/**
 * Who a widget credential belongs to.
 *
 * A discriminated union rather than two optional id fields, so that "exactly one
 * of the two" is a fact the type system carries rather than a comment every
 * consumer has to remember. The distinction is load-bearing downstream: an
 * INTERNAL identity becomes a real `Comment.authorId`, a PORTAL identity becomes a
 * `CommentExternalAuthor` shim, and `lib/comments.ts` throws if a caller supplies
 * both.
 */
export type EmbedVisitorIdentity =
  | { kind: "PORTAL"; portalAccountId: string; email: string; name: string | null }
  | { kind: "INTERNAL"; userId: string; email: string; name: string | null }
...
/**
 * The identity half of a deposit or a mint: exactly one of the two ids.
 *
 * Not `EmbedVisitorIdentity` — that type carries a resolved email and display
 * name for rendering, which a writer has no business accepting from its caller.
 * Here only the id is passed, and the profile is read back from the row it points
 * at.
 */
export type EmbedIdentityRef = { portalAccountId: string; userId?: never } | { userId: string; portalAccountId?: never }

/**
 * Narrows a stored pair of nullable ids back to the exactly-one-of invariant, or
 * fails closed.
 *
 * Reached on the read path, where the row was written by this module and should
 * already satisfy it. Checked anyway because DSQL has no CHECK constraint to hold
 * it (relationMode="prisma"), so the only thing standing between a malformed row
 * and an ambiguous identity is this function — and "both set" is the case worth
 * refusing rather than guessing at, since guessing would pick which of two people
 * a comment is attributed to.
 */
function narrowStoredIdentity(row: {
  portalAccountId: string | null
  userId: string | null
}): EmbedIdentityRef | null {
  if (row.portalAccountId && row.userId) return null
  if (row.portalAccountId) return { portalAccountId: row.portalAccountId }
 ...
 * the identity from a verified portal session or a Compass SSO session whose
 * workspace membership has been checked, and `feedbackSourceId` from a presented
 * embed token — never from a sibling request parameter, which anyone could set to
 * any value while presenting a credential for something else.
 */
export async function mintEmbedVisitorSession(input: {
  feedbackSourceId: string
  identity: EmbedIdentityRef
  now?: Date
  ...
  await getPrisma().embedVisitorSession.create({
    data: {
      ...
      portalAccountId: input.identity.portalAccountId ?? null,
 ...
 *
 * Returns null for every failure (unknown, revoked, expired, wrong source,
 * identity no longer resolvable) so a caller holding a bad credential learns only
 * that it does not work. NOT memoized with `cache()`, unlike the cookie path in
 * lib/portal-auth.ts: there is no per-request repetition to collapse here, since
 * the widget presents the token once per request.
 *
 * Resolving the identity does NOT re-check workspace membership for an internal
 * session. That check belongs to the write path (see the comments route), which
 * runs it against current state — this function's job is to say who presented the
 * token, not what they may do with it.
 */
export async function resolveEmbedVisitorToken(
  rawToken: string,
  feedbackSourceId: string
): Promise<EmbedVisitorIdentity | null> {
  ...
  // Rejects a row with both ids set or neither. Under relationMode="prisma" the
  // relation fields below can also resolve to null for a deleted account or user,
  // which is treated the same way: an identity that cannot be named cannot author.
  const stored = narrowStoredIdentity(row)
  if (!stored) return null
  if (stored.portalAccountId && !row.portalAccount) return null
  if (stored.userId && !row.user) return null
  ...
  if (row.portalAccount && stored.portalAccountId) {
    return {
      kind: "PORTAL",
  ...
  if (row.user && stored.userId) {
    return { kind: "INTERNAL", userId: row.user.id, email: row.user.email, name: row.user.name }
  }
  return null
}
...
/**
 * Records that a nonce entitles its bearer to a session for this identity and
 * source. Called from the first-party popup, which holds either a verified portal
 * session or a Compass SSO session.
 *
 * Whichever it is, the popup has already done the authorization work — a portal
 * account is verified by definition; an internal user must have been confirmed a
 * member of the source's workspace *before* this is called. This function does not
 * re-derive either, because it cannot: it sees an id, not a session.
 *
...
export async function claimEmbedAuthHandoff(input: {
  ...
  if (row.expiresAt.getTime() <= Date.now()) return null
  // Same exactly-one-of check as on the session read path. A handoff carrying both
  // or neither identity is unclaimable rather than resolved one way — and it has
  // already been burned above, so the visitor's recovery is to sign in again.
  const identity = narrowStoredIdentity(row)
  if (!identity) return null

  return mintEmbedVisitorSession({ feedbackSourceId: row.feedbackSourceId, identity })
}
