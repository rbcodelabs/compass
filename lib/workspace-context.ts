...
/**
 * The shared per-request workspace resolver.
 *
 * Deliberately a separate module from lib/workspace.ts. This file reaches
 * @/lib/session -> @/auth -> next-auth, and anything importing it inherits
 * that module graph. lib/workspace.ts is imported by plain DB-layer unit
 * tests (see __tests__/lib-workspace-orphaned-membership.test.ts), which
 * would then have to mock @/auth just to load — a cost every future consumer
 * would keep paying. Keeping the auth-aware resolver here leaves
 * lib/workspace.ts a pure query module.
 */
/**
