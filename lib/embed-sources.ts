...
import { resolveEmbedAuthMode, type EmbedAuthMode } from "@/lib/embed-auth-mode"
...
export type ResolvedEmbedSource = {
  ...
  /**
   * Which identity a commenter must present. Resolved from the stored column, so
   * callers never see null and never repeat the coalesce — see
   * lib/embed-auth-mode.ts. Like `artifactId`, this is a property of the stored
   * source row and never of the request: the widget cannot ask to be treated as
   * internal.
   */
