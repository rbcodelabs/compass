    ...
    {snapshots.map((snapshot, index) => {
      ...
      return <Collapsible key={snapshot.id} defaultOpen={index === 0} className="rounded-lg border p-4"><CollapsibleTrigger className="w-full cursor-pointer text-left text-sm font-medium">{snapshot.createdAt.toISOString().slice(0, 16).replace("T", " ")} UTC · {snapshot.sessionCount} sessions{stale ? " · New source data available" : ""}</CollapsibleTrigger><CollapsibleContent>
        {content ? <div className="mt-3 space-y-4 text-sm">
          ...
          <div><h3 className="font-semibold">Cross-session patterns</h3>{content.patterns.length ? <ul className="list-disc space-y-1 pl-5">{content.patterns.map((item, i) => <li key={i}>{item.text}</li>)}</ul> : <p className="text-text-muted">Insufficient repeated evidence to identify cross-session patterns.</p>}</div>
          <div><h3 className="font-semibold">Jobs to be done</h3><ul className="space-y-2">{content.jobs.map((job, i) => <li key={i}><p>{job.job}</p><p className="text-text-subtle">{job.context}</p></li>)}</ul></div>
          <div><h3 className="font-semibold">Recommendations</h3><ol className="list-decimal space-y-1 pl-5">{content.recommendations.map((item, i) => <li key={i}>{item.text}</li>)}</ol></div>
