...
export function PmInterviewExperience({ interviewId, orgSlug, workspaceSlug, targetType, targetTitle, omissions, initialTurns, initialProposal, initialReceipt, initialDisposition, initialGenerationState, initialReviewBaseline, initialContextFields, applicationDisabledReason, owner, voiceEnabled, initialAgentConversationId = null }: { interviewId: string; orgSlug: string; workspaceSlug: string; targetType: PmInterviewTargetType; targetTitle: string; omissions: string[]; initialTurns: Turn[]; initialProposal: Proposal | null; initialReceipt: Receipt | null; initialDisposition: string; initialGenerationState: string; initialReviewBaseline: { version: 1; fields: Record<string, string | null> }; initialContextFields: Record<string, string | null>; applicationDisabledReason: string | null; owner: boolean; voiceEnabled: boolean; initialAgentConversationId?: string | null }) {
  ...
  function openConversation(result: unknown) {
    if (!result || typeof result !== "object" || !("conversationUrl" in result) || typeof result.conversationUrl !== "string" || !result.conversationUrl.startsWith(`/${orgSlug}/${workspaceSlug}/agent?c=`)) throw new Error("The agent conversation could not be opened")
    router.push(result.conversationUrl)
  }

  async function finish() {
    setBusy(true); setError(null)
    try { const result = await api("/complete"); openConversation(result) }
    catch (caught) { setError(caught instanceof Error ? caught.message : "The agent conversation could not be opened") }
    finally { setBusy(false) }
  }
  ...
  if (initialAgentConversationId && owner) return <section className="mx-auto w-full max-w-3xl space-y-4 rounded-xl border p-5"><h2 className="text-lg font-semibold">{targetTitle}</h2><p>Your transcript is saved. Follow the update in the core agent conversation.</p><Button onClick={() => router.push(`/${orgSlug}/${workspaceSlug}/agent?c=${encodeURIComponent(initialAgentConversationId)}`)}>Open agent conversation</Button><Transcript turns={turns} />{proposal && <section><h3 className="font-medium">Previous proposal (read-only)</h3><p>{proposal.brief}</p>{fields.map(([field, change]) => <p key={field}>{field}: {change.value ?? "empty"}</p>)}</section>}<ReceiptSummary receipt={receipt} /></section>
  ...
  if (proposal) return <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_22rem]">
    <section className="space-y-5 rounded-xl border bg-surface-panel p-5"><div><p className="text-xs font-semibold uppercase tracking-wide text-primary">PM interview brief</p><h2 className="mt-2 text-lg font-semibold">{targetTitle}</h2><p className="mt-2 text-sm text-text-subtle">{proposal.brief}</p></div>
      ...
      {owner && disposition === "PENDING" && <div className="flex flex-wrap gap-2"><Button onClick={apply} disabled={busy || fields.length === 0 || generationState === "STALE" || Boolean(applicationDisabledReason)}>Apply selected changes</Button><Button variant="outline" onClick={finish} disabled={busy || Boolean(applicationDisabledReason)}>Use interview to update item now</Button><Button variant="outline" onClick={dismiss} disabled={busy}>Dismiss proposal</Button></div>}{disposition !== "PENDING" && <p className="rounded-lg bg-surface-inset p-3 text-sm">Proposal {disposition.toLowerCase()}.</p>}
    </section>
