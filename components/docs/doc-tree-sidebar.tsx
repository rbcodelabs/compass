    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-2 pb-2">
        ...
        <button
          ...
          disabled={isPending}
          className="flex items-center gap-1 text-xs text-slate-500 hover:text-slate-800 transition-colors px-1 py-0.5 rounded hover:bg-slate-100 disabled:opacity-50"
          title="New page"
        >
      ...
      <div className="flex-1 overflow-y-auto min-h-0">
        {docs.length === 0 ? (
          <p className="text-xs text-slate-400 px-2 py-1">No pages yet.</p>
        ) : (
          <DocTreeList
            items={docs}
            orgSlug={orgSlug}
