...
export function ArtifactDetail({ artifact, html, workspaceId, basePath, solutions }: ArtifactDetailProps) {
  ...
  return <div className="mx-auto max-w-5xl p-4 sm:p-8 space-y-6">
    <header className="flex items-start justify-between gap-4">
      <div><div className="text-xs font-medium uppercase tracking-wide text-indigo-600">Artifact · {artifact.sourceType === "EXTERNAL_LINK" ? "External" : "HTML prototype"}</div><h1 className="text-2xl font-semibold text-slate-900">{artifact.title}</h1>{artifact.description && <p className="mt-1 text-slate-600">{artifact.description}</p>}</div>
    ...
    {artifact.status === "ARCHIVED" && <div className="rounded-md bg-amber-50 p-3 text-sm text-amber-800">This artifact is archived.</div>}
    ...
    <section className="rounded-lg border p-4 space-y-3"><h2 className="font-semibold">Linked solutions</h2>{linked.length === 0 ? <p className="text-sm text-slate-500">Not linked to a solution.</p> : <ul className="space-y-2">{linked.map((solution) => <li key={solution.id} className="flex items-center justify-between gap-2"><span>{solution.title}</span><Button size="sm" variant="ghost" onClick={() => run(() => unlinkArtifact(workspaceId, artifact.id, solution.id, basePath))}>Unlink</Button></li>)}</ul>}
    ...
    <Link href={basePath} className="text-sm text-indigo-600">Back to Docs</Link>
  </div>
}
