        ...
        return (
          <Link
            ...
            className={cn(
              ...
              isActive ? "text-indigo-400" : "text-slate-500 hover:text-slate-300 active:text-slate-200"
            )}
            aria-current={isActive ? "page" : undefined}
          >
            {isActive && (
              <span
                className="absolute top-0 left-1/2 -translate-x-1/2 h-0.5 w-8 bg-indigo-400 rounded-full"
                aria-hidden="true"
              />
            )}
            <Icon
