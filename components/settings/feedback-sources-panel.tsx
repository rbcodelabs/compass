...
import { DEFAULT_EMBED_AUTH_MODE, type EmbedAuthMode } from "@/lib/embed-auth-mode"

/**
 * The operator-facing wording for each mode, and the one place it lives.
 *
...
const AUTH_MODE_OPTIONS: { value: EmbedAuthMode; label: string; hint: string }[] = [
  {
    value: "INTERNAL_SSO",
    label: "Your team, signed in with Compass SSO",
    hint: "Comments are filed under the commenter's real Compass account. Only members of this workspace can comment.",
  },
  {
    value: "PORTAL",
...
export type FeedbackSourceRow = {
  ...
  enabled: boolean
  /**
   * Already resolved by the server — a stored NULL arrives here as INTERNAL_SSO,
   * so nothing in this file has to know the column is nullable.
   */
...
type Props = {
  ...
  /**
   * `Workspace.artifactFeedbackPublic`, the initial state of the toggle below.
   *
   * Workspace-wide rather than per source, because the column is. It controls
   * *anonymous* reading only: since the widget's read path became identity-aware,
   * a workspace member signed in through Compass SSO reads the history whether
   * this is on or off, so the toggle is now about publishing to strangers rather
...
export function FeedbackSourcesPanel({
  ...
  const [publicReads, setPublicReads] = useState(artifactFeedbackPublic)
  const [publicReadsError, setPublicReadsError] = useState<string | null>(null)

  // Create form
  const [newName, setNewName] = useState("")
  const [newArtifactId, setNewArtifactId] = useState("")
  const [newOrigins, setNewOrigins] = useState("")
  // Pre-selected rather than left empty: this is the mode that works on this
  // deployment, and an operator who never touches the control gets a usable source.
  const [newAuthMode, setNewAuthMode] = useState<EmbedAuthMode>(DEFAULT_EMBED_AUTH_MODE)
  ...
  function handleCreate() {
    ...
    startTransition(async () => {
      ...
      setNewArtifactId("")
      setNewOrigins("")
      setNewAuthMode(DEFAULT_EMBED_AUTH_MODE)
    })
  }

  /**
   * Saved immediately on change rather than behind a Save button, matching the
   * enabled switch above and unlike the origins textarea — a select has no
   * half-typed state to protect. Optimistic for the same reason, and rolled back
   * the same way.
   */
  function handleChangeAuthMode(source: FeedbackSourceRow, authMode: EmbedAuthMode) {
    if (authMode === source.authMode) return
    const previous = source.authMode
    setRowError(source.id, null)
    setSources((prev) => prev.map((s) => (s.id === source.id ? { ...s, authMode } : s)))
    startTransition(async () => {
      const result = await updateFeedbackSource(orgSlug, workspaceSlug, source.id, { authMode })
      if (!result.ok) {
        setSources((prev) => prev.map((s) => (s.id === source.id ? { ...s, authMode: previous } : s)))
        return setRowError(source.id, result.error)
      }
      // The server's canonical value, for the same reason origins are replaced
      // rather than kept: what gates sign-in is what came back.
      setSources((prev) => prev.map((s) => (s.id === source.id ? { ...s, authMode: result.authMode } : s)))
    })
  }

  /**
   * Optimistic and rolled back on failure, the same shape as the per-source
   * switches — a switch that waits for a round trip reads as broken.
   */
  function handleTogglePublicReads(next: boolean) {
    setPublicReadsError(null)
    setPublicReads(next)
    startTransition(async () => {
      const result = await setArtifactFeedbackPublic(orgSlug, workspaceSlug, next)
      if (!result.ok) {
        setPublicReads(!next)
        return setPublicReadsError(result.error)
      }
      // The server's canonical value, for the same reason origins are replaced
      // rather than kept: what gates reads is what came back.
      setPublicReads(result.artifactFeedbackPublic)
    })
  }
    ...
    <div className="flex flex-col gap-5">
      <div className="flex flex-col gap-1.5 rounded-xl border border-border bg-card px-4 py-3.5">
        <div className="flex items-start justify-between gap-4">
          <Label htmlFor="artifact-feedback-public" className="text-sm font-medium">
            Let anyone with the embed code read existing feedback
          </Label>
          <Switch
            id="artifact-feedback-public"
            data-testid="artifact-feedback-public-toggle"
            aria-label="Let anyone with the embed code read existing feedback"
            checked={publicReads}
            disabled={isPending}
            onCheckedChange={handleTogglePublicReads}
          />
        </div>
        {/* States the consequence, not the mechanism. The second sentence is the
            one that stops an operator flipping this by mistake: the case they
            actually have in mind — their own team reading the thread — already
            works without it. */}
        <p className="text-xs text-muted-foreground">
          On: anyone who can load a page carrying one of this workspace&apos;s embed tokens can read every
          comment filed through it, without signing in and without being a member here. Off: only signed-in
          members of this workspace can read the history.
        </p>
        <p className="text-xs text-muted-foreground">
          You do not need this for your own team. A source set to &ldquo;Your team, signed in with Compass
          SSO&rdquo; already shows the full history to workspace members, whichever way this is set — turn it on
          only when the feedback itself is meant to be public.
        </p>
        {publicReadsError && <p className="text-xs text-status-danger">{publicReadsError}</p>}
      </div>

      {/* Narrowed from "whenever reads are unpublished": an internal-SSO source is
          no longer affected, because its commenters are workspace members and the
          read path now authorizes them directly. An external reviewer is never a
          member, so for PORTAL sources the old warning is still exactly true —
          signing in by email link authorizes writing, not reading. */}
      {!publicReads && sources.some((source) => source.authMode === "PORTAL") && (
        <p className="rounded-md border border-border bg-muted px-3 py-2 text-xs text-muted-foreground">
          One or more sources below let external reviewers comment by email link. Those reviewers cannot see
          earlier comments while this is off — signing in as an external reviewer does not grant read access.
          They can still leave new feedback.
        </p>
      )}
        ...
        return (
          ...
          >
            ...
            <div className="flex flex-col gap-1.5">
              <Label htmlFor={`auth-mode-${source.id}`} className="text-xs">
                Who can comment
              </Label>
              <Select
                value={source.authMode}
                onValueChange={(value) => handleChangeAuthMode(source, value as EmbedAuthMode)}
              >
                <SelectTrigger
                  id={`auth-mode-${source.id}`}
                  data-testid={`feedback-source-auth-mode-${source.id}`}
                  aria-label={`Who can comment on ${source.name}`}
                  disabled={isPending}
                  className="w-full"
                >
                  <SelectValue />
      ...
      {artifacts.length === 0 ? (
        ...
        <div className="flex flex-col gap-3 rounded-xl border border-border border-dashed px-4 py-3.5">
          ...
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="new-feedback-source-auth-mode" className="text-xs">
              Who can comment
            </Label>
            <Select value={newAuthMode} onValueChange={(value) => setNewAuthMode(value as EmbedAuthMode)}>
              <SelectTrigger
                id="new-feedback-source-auth-mode"
