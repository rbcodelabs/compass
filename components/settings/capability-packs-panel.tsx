...
export type CapabilityPackSettingsRow = { packId: string; displayName: string; enabled: boolean; selectedVersionId: string; enabledSkillIds: string[]; versions: Array<{ id: string; version: string; commit: string; digest: string; skills: Array<{ id: string; enabledByDefault?: boolean }> }> }
...
export function CapabilityPacksPanel({ orgSlug, workspaceSlug, initialPacks }: { orgSlug: string; workspaceSlug: string; initialPacks: CapabilityPackSettingsRow[] }) {
  ...
  return <div className="space-y-6">
    ...
    {initialPacks.map((row) => { const selected = row.versions.find((v) => v.id === row.selectedVersionId) ?? row.versions[0]; return <div key={row.packId} className="space-y-3 rounded-lg border border-border-default p-4">
