...
(function () {
  ...
  function renderThreads() {
    ...
    if (state.readBlocked) {
      // Two different true statements, and which one holds depends on who asked.
      // The server's 403 is deliberately non-committal — it will not confirm to a
      // caller who may not read a thread that one exists — so the widget picks
      // between them using the one thing it already knows locally: whether we hold
      // an identity.
      //
      // Signed in and still refused means the workspace has not published the
      // thread and this account cannot read it either. Not signed in means signing
      // in *might* help, and the wording is hedged on purpose: it helps a member of
      // the workspace, while an external reviewer signing in by email link gains
      // the right to write, not to read.
      ui.body.appendChild(
        make(
          "p",
          { "class": "empty", "data-compass": "read-blocked" },
          state.identity
            ? "Earlier comments are not published for this workspace, so they are hidden here. You can still leave new feedback."
            : "Earlier comments are hidden here. If you have a Compass account for this workspace, sign in to see them — either way you can still leave new feedback."
        )
      );
      return;
    }
  ...
  function signOut() {
    ...
    request("/session", { method: "DELETE" }).then(
      guarded("signout-done", function () {
        ...
        // The mirror of the sign-in case: a thread we were shown as a member is not
        // ours to display once the credential is gone. Dropped and re-asked, which
        // for an unpublished workspace lands on the banner.
        rereadThread();
      })
    );
  }
  ...
  /**
   * Re-asks for the thread after the identity changed.
   *
   * The latch below is keyed on a 403 the server gave to whoever we were *then*.
   * Since the read path became identity-aware that answer is no longer a property
   * of the workspace alone, so signing in or out invalidates it: without this, a
   * member who signs in mid-session keeps staring at the "hidden" banner until they
   * reload the page, and someone who signs out keeps a thread on screen they can no
   * longer fetch.
   *
   * Deliberately not on a timer and not on panel open — only on the two events that
   * can actually change the answer, so the read quota is still spent once per state
   * rather than once per look.
   */
  function rereadThread() {
  ...
  function refreshComments() {
    // A 403 is a standing configuration answer for this identity, not a transient
    // failure, so it is latched. Re-asking on every panel open would spend the read
    // quota discovering the same thing forever. Cleared by rereadThread when the
    // identity changes, which is the one thing that can change the answer.
