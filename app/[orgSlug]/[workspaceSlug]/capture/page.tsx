...
export default async function CapturePage({ params, searchParams }: { params: Promise<{ orgSlug: string; workspaceSlug: string }>; searchParams?: Promise<{ page?: string; archived?: string }> }) {
  ...
  const studies = await prisma.researchStudy.findMany({ where: { workspaceId: workspace.id, status: archived ? "ARCHIVED" : { not: "ARCHIVED" } }, orderBy: [{ createdAt: "desc" }, { id: "desc" }], skip: (page - 1) * 20, take: 21, include: { _count: { select: { sessions: true } } } })
