...
export default async function NewArtifactPage({ params }: { params: Promise<{ orgSlug: string; workspaceSlug: string }> }) {
  ...
  return <div className="mx-auto max-w-2xl p-4 sm:p-8"><h1 className="text-2xl font-semibold mb-2">New artifact</h1><p className="text-sm text-slate-600 mb-6">Add a self-contained HTML prototype or link to an externally hosted artifact.</p><ArtifactCreateForm workspaceId={workspace.id} basePath={basePath} /></div>
}
