...
export async function POST(request: Request, { params }: { params: Promise<{ interviewId: string }> }) {
  try {
    ...
    if (base.some(key => typeof body[key] !== "string") || (body.action !== "FINAL" && body.action !== "DISCONNECT")) return NextResponse.json({ error: "Invalid request" }, { status: 400 })
    const allowed = new Set(base); if (body.action === "FINAL") ["clientEventId", "reportedOrdinal", "role", "content"].forEach(key => allowed.add(key))
