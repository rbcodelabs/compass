 ...
 *
 * That includes the submit path's write-time membership re-check for an internal
 * visitor. Deliberately not skipped as "only an image": this endpoint is the one
 * that spends Compass's blob storage, and a session that outlived its membership
 * must not be able to do that any more than it can leave a comment.
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    // See the header: current membership, not membership as of sign-in. Kept
    // before the body read for the same reason the visitor check is.
