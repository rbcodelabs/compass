...
Omit baseline/followup to start tracking without a baseline (default: last 30 completed UTC days for Vercel). A rolling followup policy is `{version:1,mode:"rolling",days:30}`; Vercel supports 7, 30, and 90 days, while Active Discovery Teams accepts only 30. To compare periods, pass both fixed inclusive UTC windows (`since`/`until` dates, each 1–90 days). To replace a comparison with tracking, pass `baseline:null` plus a rolling followup. Binding DTOs distinguish tracking/comparison; tracking refreshes produce only a current observation. Vercel definitions support pageviews, daily visitors and named event counts, with structured path/property/flag filters. Do not sum daily uniques. Native Active Discovery Teams is operator-workspace-only, prospective and partial for the first 30 days; it remains a current trailing 30×24-hour snapshot. Never equate unavailable/stale data with zero or automatically overwrite experiment conclusions or KR check-ins. Example query: `{metric:"event_count",eventName:"compass_activity",eventProperties:{action:"result_recorded"}}`.
