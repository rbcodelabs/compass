...
CREATE TABLE research_voice_commands (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), voice_call_id UUID NOT NULL, session_id UUID NOT NULL, idempotency_key VARCHAR(128) NOT NULL, kind VARCHAR(30) NOT NULL, attachment_id UUID, status VARCHAR(20) NOT NULL DEFAULT 'PENDING', attempt_count INTEGER NOT NULL DEFAULT 0, claim_expires_at TIMESTAMPTZ, error_code VARCHAR(50), canonical_command_id UUID, created_at TIMESTAMPTZ NOT NULL DEFAULT now(), updated_at TIMESTAMPTZ NOT NULL DEFAULT now());
